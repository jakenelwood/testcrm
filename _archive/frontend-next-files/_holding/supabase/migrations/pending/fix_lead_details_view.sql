-- Fix the lead_details view to correctly join all necessary tables
-- This will ensure both the modal view and detailed view have access to all lead information
...
-- Create the comprehensive lead_details view with all necessary joins
...
SELECT 
  ...
  CASE 
    WHEN c.lead_type = 'Business' THEN c.name
    ELSE split_part(c.name, ' ', 1)
  END AS first_name,
  
  CASE 
    WHEN c.lead_type = 'Business' THEN NULL
    ELSE substring(c.name from position(' ' in c.name) + 1)
  END AS last_name,
  
  -- Add email and phone_number for backward compatibility
  c.email AS email,
