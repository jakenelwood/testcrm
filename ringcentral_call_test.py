...
try:
    ...
    try:
        # Try to get platform info to check if we're authenticated
        platform_info = platform.get('/restapi/v1.0').json()
        print(f"Already authenticated. Server version: {platform_info.get('version')}")
    except Exception as auth_error:
        print(f"Not authenticated. Error: {str(auth_error)}")
